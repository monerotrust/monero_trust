...
#include "ui_zxmtcontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zxmtTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zxmtTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zXMT.";
            else if (!mint.isSeedCorrect)
                strReason = "The zXMT seed used to mint this zXMT is not the same as currently hold in the wallet";
            else
