...
/**
 * MoneroTrust RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZxmtSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzxmtseed(const UniValue& params, bool fHelp);
extern UniValue getzxmtseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzxmt(const UniValue& params, bool fHelp);
extern UniValue dzxmtstate(const UniValue& params, bool fHelp);
