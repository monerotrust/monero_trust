...
bool ContextualCheckZerocoinStake(int nPreviousBlockHeight, CStakeInput* stake)
{
    if (nPreviousBlockHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zXMT stake block is less than allowed start height", __func__);

    if (CZPivStake* zXMT = dynamic_cast<CZPivStake*>(stake)) {
        CBlockIndex* pindexFrom = zXMT->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zXMT stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zXMT stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zXMT->GetValue()));
        if (nChecksum200 != zXMT->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zXMT->GetChecksum(), nChecksum200);
    } else {
...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake, int nPreviousBlockHeight)
{
    ...
    if (txin.IsZerocoinSpend()) {
        ...
        if (!ContextualCheckZerocoinStake(nPreviousBlockHeight, stake.get()))
            return error("%s: staked zXMT fails context checks", __func__);
    } else {
        ...
        CPivStake* xmtInput = new CPivStake();
        xmtInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(xmtInput);
    }
    ...
    if (!txin.IsZerocoinSpend() && nPreviousBlockHeight >= Params().Zerocoin_Block_Public_Spend_Enabled() - 1) { //Equivalent for zXMT is checked above in ContextualCheckZerocoinStake()
