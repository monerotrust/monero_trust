...
CzXMTWallet::CzXMTWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zxmt seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZXMTSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZXMTSeed_deprecated()) {
                    LogPrintf("%s: Updated zXMT seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zxmt seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zxmt wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzXMTWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZXMTCount(nCountLastUsed);
    else if (!walletdb.ReadZXMTCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzXMTWallet::Lock()
{
    seedMaster = 0;
}

void CzXMTWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzXMTWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZXMT(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzXMTWallet::LoadMintPoolFromDB()
{
...
void CzXMTWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzXMTWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzXMTWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zxmtTracker->HasPubcoinHash(pMint.first)) {
...
bool CzXMTWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZXMT(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zxmtTracker which also adds to database
    pwalletMain->zxmtTracker->Add(dMint, true);
...
void CzXMTWallet::SeedToZXMT(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzXMTWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzXMTWallet::GenerateDeterministicZXMT(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzXMTWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzXMTWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzXMTWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
