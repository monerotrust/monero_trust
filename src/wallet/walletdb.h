...
public:
    ...
    bool WriteZXMTSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZXMTSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZXMTSeed_deprecated(uint256& seed);
    bool EraseZXMTSeed();
    bool EraseZXMTSeed_deprecated();

    bool WriteZXMTCount(const uint32_t& nCount);
    bool ReadZXMTCount(uint32_t& nCount);
