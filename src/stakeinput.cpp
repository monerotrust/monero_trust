...
// The zXMT block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zXMT is a hash of the serial
...
{
    //Create an output returning the zXMT that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZXMTOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zXMT output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zXMT", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZXMTOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zXMT output", __func__);
...
{
    CzXMTTracker* zxmtTracker = pwallet->zxmtTracker.get();
    CMintMeta meta;
    if (!zxmtTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zxmtTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a XMT stake is the outpoint
